...
{
	//printf("enter DFA to find match url.\n");
	char *newUrl;
	int bool;      // 判重
	...
	while (*sourceCode != '\0')
	{
		...
		case 8:
			...
			case '/':
				...
				char temp[20];
				memcpy(temp, sourceCode, 12);       // 匹配 news.stnn.cc
				*(temp + 12) = '\0';
				if (0 == strcmp(temp, "news.stnn.cc"))
				{
					state = 9;
					sourceCode = sourceCode + 12;   // 跳过 news.stnn.cc
					start = sourceCode;
