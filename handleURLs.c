        ...
        while (flag)
        {
            ...
            for (i = 0; i < events_num; ++i)
            {
                if (events[i].events & EPOLLIN)     //接收到数据，读socket
                {
                    ...
                    if(code == 200)
                    {
                    	//printf("get source code success!\n");
                    	if(strlen(arg->urlName) < 300) {
                    		URLs[urlsNum] = arg->urlName;
                        	printf("url name is %s\n", arg->urlName);
                        	fprintf(url, "%d http://news.stnn.cc%s\n", urlsNum, arg->urlName);   // 写入 url.txt 文件
                        	// 在此处加入到树中
...
int buildConnect(int *sockfd, char* serverip, int port)
{
    ...
    struct hostent *host;  
	if(0==(host=gethostbyname(serverip))){
	...
	addr.sin_addr.s_addr = ((struct in_addr*)(host->h_addr))->s_addr; // 按照网络字节顺序存储IP地址
